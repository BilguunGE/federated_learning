...
class LFUCache(object):
    ...
    def get_state(self):
        #print(self.cache)
        #state = [[k,v.freq_node.freq] for k, v in sorted(self.cache.items(), key=lambda item: item[1].freq_node.freq)]
        #print(state)
        if len(self.cache) == 0:
            return [0,0]
        if len(self.cache) == 1:
            l = [ord(data) for data in [*self.cache]]
            l.append(0)
            return l
        return [ord(data) for data in [*self.cache]]
